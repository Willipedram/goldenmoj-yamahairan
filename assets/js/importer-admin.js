(function () {
    ...
    ready(function () {
        ...
        form.addEventListener('submit', function (event) {
            ...
            if (!urls.length) {
                window.alert(settings.i18n && settings.i18n.invalidRequest ? settings.i18n.invalidRequest : 'Please add at least one URL.');
                return;
            }
            ...
            resetLog();
            setProgress(0, settings.i18n ? settings.i18n.ready : 'Waiting to start…');
            ...
            function processNext() {
                if (index >= total) {
                    setProgress(100, settings.i18n ? settings.i18n.complete : 'Completed');
                    appendLog(settings.i18n ? settings.i18n.complete : 'All products imported successfully.', hasErrors ? 'is-error' : 'is-success');
                    setFormDisabled(false);
                    return;
                }

                var currentNumber = index + 1;
                var statusLabel = settings.i18n && settings.i18n.processing ? format(settings.i18n.processing, [currentNumber, total]) : 'Processing…';
                ...
                fetch(settings.ajaxUrl, {
                    ...
                    .catch(function (error) {
                        hasErrors = true;
                        appendLog(error.message || (settings.i18n ? settings.i18n.error : 'An unexpected error occurred.'), 'is-error');
                        index += 1;
                        processNext();
                    });
            }
